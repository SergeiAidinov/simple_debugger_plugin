    ...
    private <K,V> TableViewerColumn createColumn(String title, int width,
        ...
        column.setLabelProvider(new ColumnLabelProvider() {
            ...
            public Image getImage(Object element) {
                ...
                InnerElementRepresentationDTO dto = (InnerElementRepresentationDTO) typedPair.getSecond();
                Image img = imageExtractor.apply(typedPair);
                if (img == null) return null;

                TableItem item = findTableItem(dto);
                if (item != null) {
                    item.setData("tooltip_col_" + 1, img == getIcon(dto)
                            ? SimpleDebugerWindowsManager.instance().icons.get("inspectIcon").getSecond()
                            : SimpleDebugerWindowsManager.instance().icons.get("lens").getSecond());
                }
                return img;
            }
        });
