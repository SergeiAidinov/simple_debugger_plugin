...
public class CollectionInspectorTab implements InspectorTab {
	...
	public void showPage(AbstractInspectionDTO abstractInspectionDTO) {
		if (!(abstractInspectionDTO instanceof ArrayPageDTO)) return;
	...
	private <K, V> TableViewerColumn createColumn(String title, int width,
		...
		column.setLabelProvider(new ColumnLabelProvider() {
			...
			public Image getImage(Object element) {
				...
				if (!(typedPair.getSecond() instanceof InnerElementRepresentationDTO dto))
					return null;
	...
	private TableItem findTableItem(InnerElementRepresentationDTO dto) {
		for (TableItem item : viewer.getTable().getItems()) {
			if (item.getData() instanceof PairDTO<?, ?> pair && pair.getSecond() == dto)
				return item;
		}
		return null;
	}
